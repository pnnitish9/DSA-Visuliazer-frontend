...
import { Layers, Pause, Play, RefreshCw, ArrowUp, ArrowDown, Eye } from 'lucide-react';
  ...
  <style>{`
    ...
    * {
    ...
    .box {
      ...
      border-bottom: none; /* Remove bottom border for stacking */
    }
    
    .stack-item:first-child .box {
      border-bottom: 2px solid var(--border-gray-500); /* Add bottom border to last item */
    }
    
    .stack-item:last-child .box {
    ...
    /* Deleting state is handled by .stack-item.deleting */
...
export default function Stack() {
  ...
  const [status, setStatus] = useState("Stack is empty. Push a value to start.");
  ...
  const handlePush = async () => {
    ...
    setExecutionLog(prev => [...prev, `Pushing ${val} onto the stack.`]);
    setStatus(`Pushing ${val}...`);
    ...
    setStack(prev => prev.map(n => n.id === newNode.id ? { ...n, state: 'default' } : n));
  ...
  const handlePop = async () => {
    ...
    setExecutionLog(prev => [...prev, `Popping ${itemToPop.value} from the stack.`]);
    setStatus(`Popping ${itemToPop.value}...`);
    
    // Set 'deleting' state for animation
    setStack(prev => prev.map(n => n.id === itemToPop.id ? { ...n, state: 'deleting' } : n));
    
    await sleep(speed); // Wait for animation
    
    if (isCancelledRef.current) return;
    
    setStack(prev => prev.filter(n => n.id !== itemToPop.id));
    setStatus(`Popped ${itemToPop.value}.`);
    setIsVisualizing(false);
  };
  ...
  const handlePeek = async () => {
    ...
    const itemToPeek = stack[stack.length - 1];
    setExecutionLog(prev => [...prev, `Peeking at top value: ${itemToPeek.value}`]);
    setStatus(`Peeking... Top value is ${itemToPeek.value}`);
    
    setStack(prev => prev.map(n => n.id === itemToPeek.id ? { ...n, state: 'visiting' } : n));
    
    await sleep(speed * 2); // Hold highlight longer
    
    if (isCancelledRef.current) return;
    
    setStack(prev => prev.map(n => n.id === itemToPeek.id ? { ...n, state: 'default' } : n));
  ...
  const handleReset = () => {
    ...
    setValue("");
    setError(null);
    setStatus("Stack is empty. Push a value to start.");
  ...
  const statusColor = status.includes("Popped") || status.includes("empty")
    ? "status-not-found"
    : status.includes("Pushed") || status.includes("Peeked")
    ? "status-found"
    ...
    <div className="visualizer-container">
      ...
      <aside className="controls-sidebar">
        ...
        <div className="actions-grid">
            <button onClick={handlePush} disabled={isVisualizing} className="btn btn-green">
                <ArrowUp size={18} />
                Push
            </button>
            <button onClick={handlePop} disabled={isVisualizing || stack.length === 0} className="btn btn-red">
                <ArrowDown size={18} />
                Pop
            </button>
        </div>
        <div className="actions-grid">
            <button onClick={handlePeek} disabled={isVisualizing || stack.length === 0} className="btn btn-cyan">
                <Eye size={18} />
                Peek
            </button>
             <button
      ...
      <main className="main-content">
        ...
        <section className="visualization-section">
          ...
          <div className="visualization-boxes">
            {stack.length === 0 && <span style={{color: 'var(--text-gray-500)', alignSelf: 'center'}}>Stack is empty.</span>}
            
            <div className="stack-container">
                {stack.map((node, idx) => (
                  <div 
                    className={`stack-item ${node.state === 'deleting' ? 'deleting' : ''}`} 
                    key={node.id}
                  >
                    <div
                    ...
                    {idx === stack.length - 1 && 
                        <span className="top-pointer">&larr; TOP</span>
                    }
                  </div>
                ))}
            </div>
