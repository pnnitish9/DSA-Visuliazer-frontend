...
import { SortAsc, Pause, Play, RefreshCw, Shuffle } from 'lucide-react';
...
/**
 * The core visualization logic for Bubble Sort.
 */
async function visualizeBubbleSort(
  ...
  setExecutionLog(["Visualization started..."]);
  await sleep(speed);

  // Outer loop
  for (let i = 0; i < n; i++) {
    if (isCancelledRef.current) break;
    setHighlightLineNum(4); // for i in range(n):
    await checkPause();
    await sleep(speed);

    let swapped = false;
    setHighlightLineNum(5); // swapped = False
    await checkPause();
    await sleep(speed);
    ...
    for (let j = 0; j < n - i - 1; j++) {
      if (isCancelledRef.current) break;
      setHighlightLineNum(7); // for j in range(0, n-i-1):
      await checkPause();

      // 1. Mark as comparing
      const compareMsg = `Comparing: [${j}] ${boxes[j].value} and [${j+1}] ${boxes[j+1].value}`;
      setStatus(compareMsg);
      setExecutionLog(prev => [...prev, compareMsg]);
      boxes[j].state = 'comparing';
      boxes[j+1].state = 'comparing';
      // Reset any leftover translations
      boxes[j].translateX = 0; 
      boxes[j+1].translateX = 0;
      setBoxes([...boxes]);
      setHighlightLineNum(9); // if arr[j] > arr[j+1]:
      await sleep(speed);
      ...
      if (boxes[j].value > boxes[j+1].value) {
        swapped = true;
        setHighlightLineNum(11); // arr[j], arr[j+1] = arr[j+1], arr[j]
        
        // 2a. Mark as swapping & set translations
        const swapMsg = `Swap: ${boxes[j].value} with ${boxes[j+1].value}`;
        setStatus(swapMsg);
        setExecutionLog(prev => [...prev, swapMsg]);
        boxes[j].state = 'swapping';
        boxes[j+1].state = 'swapping';
        boxes[j].translateX = '4rem'; // 3.5rem box + 0.5rem gap
        boxes[j+1].translateX = '-4rem';
        ...
        // No swap needed
        const noSwapMsg = `No swap needed.`;
        setStatus(noSwapMsg);
        setExecutionLog(prev => [...prev, noSwapMsg]);
        await sleep(speed); // Wait for compare duration
        
        // Reset state to default
        boxes[j].state = 'default';
        boxes[j+1].state = 'default';
        boxes[j].noTransition = false; // Ensure transition is on
        boxes[j+1].noTransition = false; // Ensure transition is on
        setBoxes([...boxes]);
      }

      // 3. Reset state after compare/swap
      // boxes[j].state = 'default';
      // boxes[j+1].state = 'default';
      // setBoxes([...boxes]);
    }
    ...
    // Mark the last element of the inner loop as sorted
    if (n - 1 - i >= 0 && boxes[n - 1 - i]) {
      boxes[n - 1 - i].state = 'sorted';
      setBoxes([...boxes]);
      setExecutionLog(prev => [...prev, `Element ${boxes[n-1-i].value} is sorted.`]);
    }

    setHighlightLineNum(14); // if not swapped:
    await sleep(speed);
    if (!swapped) {
      setHighlightLineNum(15); // break
      setExecutionLog(prev => [...prev, "Array is sorted. Early exit."]);
      await sleep(speed);
      break; // Optimization
    }
  }
